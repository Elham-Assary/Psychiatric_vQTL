#######################################################################################################
#Format results: get hwe per ancestry and combine with vQLT results and INFO (for autosomes and Xchromosome males/females if available)
#######################################################################################################
...
info_file <- "INFO.txt"               # path to INFO file, with two columns: SNP, INFO
## also need ancestry.keep files for relevant ancestries, to get hwe in step1 (file name is assumed to be in the format of ancestrycode.keep, e.g. EUR.keep)
...
combine_vqtl <- function(drm_file, hwe_file = NULL, info_file = NULL) {
  ...
  if (!is.null(hwe_file)) {
    ...
    if ("MIDP" %in% names(hwe_dt)) {
      hwe_dt <- hwe_dt[, .(SNP = ID, HWE = MIDP)]
    } else if ("P" %in% names(hwe_dt)) {
      hwe_dt <- hwe_dt[, .(SNP = ID, HWE = P)]
    } else {
      stop("No MIDP or P column found in ", hwe_file,
...
for (ancestry in ancestries) {
  ...
  cat("Running PLINK2 --hardy midp (autosomes + X) for ancestry", ancestry, "...\n")
